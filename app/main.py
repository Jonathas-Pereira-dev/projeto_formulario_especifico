...
load_dotenv(dotenv_path="dabase.env")
...
#CONFIG
SECRET_KEY = os.environ.get("SECRET_KEY", "mude_para_producao_gerar_com_openssl")
ALGORITHM = "HS256"
ACCESS_TOKEN_EXPIRE_MINUTES = 60 * 24 
DATABASE_URL = os.environ.get("DATABASE_URL", "sqlite:///./users.db")

COOKIE_NAME = "access_token"  # nome do cookie
COOKIE_SECURE = os.environ.get("COOKIE_SECURE", "false").lower() == "true"  # true em produção com HTTPS
...
PLANILHA = "FT-5.82.AD.BA6XX-403 - ANEXO 1.xlsm"  # sua planilha (mesmo nome que já tinha)
...
async def post_login(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if not user or not verify_password(password, user.hashed_password):
        # volta para /login com erro simples via query param (você pode melhorar)
        return templates.TemplateResponse("login.html", {"request": request, "error": "Usuário ou senha inválidos"})
    token = create_access_token({"sub": user.username})
    response = RedirectResponse(url="/", status_code=303)
    # cookie HTTP-only (em produção usar secure=True)
...
async def pagina_inicial(request: Request):
    ...
    return templates.TemplateResponse("index.html", {"request": request, "abas": abas, "user": {"username": user.username}})
...
async def exibir_formulario(request: Request, aba_id: str):
    ...
    return templates.TemplateResponse("formulario.html", {"request": request, "itens": itens, "headers": headers, "aba_id": aba_id, "user": {"username": user.username}})
...
async def enviar_formulario(request: Request):
    # proteger: só usuário logado pode enviar
    user = get_current_user_from_request(request)
    if not user:
        return RedirectResponse(url="/login", status_code=303)
